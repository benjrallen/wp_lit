...
if (isset($_POST["token"]) && isset($_POST["gateway"])) {
	...
	if ($_POST["gateway"] == "paypal") {
		...
		$params = array(
			...
			"notify_url" => site_url("/private/?action=confirm&token=".$order["token"]),//Only used with IPN. An internet URL where IPN form posts will be sent
			//"return" => site_url("/reserve/?action=return&token=".$order["token"]),
		  "return" => site_url("/home/"),
			"cancel_return" => site_url("/private/?action=return"),//An internet URL where your customer will be returned after cancelling payment
      "custom" => $order["token"],
			"item_name" => "C-1 Reservation",
			"item_number" => "C1-001",
			"image_url" => site_url("/img/paypal-lit-logo.png"),
...
/*		
		$params = array(
			...
			//"address_override" => 1,
			"country" => $order["country"],
			"notify_url" => site_url("/private/?action=confirm&token=".$order["token"]),
			"return" => site_url("/private/?action=return&token=".$order["token"]),
			"item_name" => "C-1 Reservation",
			"item_number" => "C1-001",
			"image_url" => site_url("/img/paypal-lit-logo.png"),
			"business" => "dan@litmotors.com",
			"hosted_button_id" => "ZHBKSR5KM42AL"
		);
*/		
    ...
    // $merchant_key = GW_SANDBOX ? "lgFj5GaFwVmf7_xyI2BeGw" : "chqag_0OYwwmPdGquUQCaw";  // Your Merchant Key
		...
		$buildLink = sprintf(GW_SANDBOX ? $sandbox : $production, $merchant_id);
	      ...
	      //$cart->SetContinueShoppingUrl("https://litmotors.com/private/?action=return");
	      $cart->SetContinueShoppingUrl("http://litmotors.com/private/?action=return");
