...
//	add_action( 'wp_enqueue_scripts', 'wpcf7_enqueue_scripts' );
	add_action( 'wp_print_styles', 'wpcf7_enqueue_scripts' );
...
function wpcf7_enqueue_scripts() {
	...
	//ba - also need to make this work differently.  we will load things using $script.js and the system set up in my bootstrap file (boot.js).
	?>
	<script type="text/javascript">
		try{
<?php /* ?>
			$script.ready( ['jquery'], function(){
				$script('<?php echo wpcf7_plugin_url( "jquery.form.js" ); ?>', 'jquery-form');
				$script.ready(['jquery-form'], function(){
					$script('<?php echo wpcf7_plugin_url( "scripts.js" ); ?>', 'contact-form-7');
				});
			});
<?php */ ?>
			Modernizr.load([
				{ load : '<?php echo wpcf7_plugin_url( "jquery.form.js" ); ?>' },
				{ load : '<?php echo wpcf7_plugin_url( "scripts.js" ); ?>' }
			]);
						
		} catch(e) {
			console.log('something has gone wrong loading the scripts.  check the function in contact form\'s plugin\'s controller.php, around line 250');	
		}
	</script>
	<?php
...
//	return wp_script_is( 'contact-form-7' );
	//ba - I changed this to always return true because I am loading the scripts asynchronously using $script.js (see boot.js for the site's bootstrap file)
	return true;
}
